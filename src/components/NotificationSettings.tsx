...
export const NotificationSettings: React.FC<NotificationSettingsProps> = ({ user }) => {
  ...
  const fetchSettings = async () => {
    try {
      const res = await api.users.getById(user.id);
      const userData = res.data?.user as any;
      if (res.success && userData && userData.settings?.notificationSettings) {
        setSettings({ notificationSettings: userData.settings.notificationSettings });
      }
    } catch (err) {
  ...
  const updateSettings = async (newFullSettings: any) => {
    setSettings(newFullSettings);

    try {
      await api.users.update(user.id, { settings: newFullSettings } as any);
    } catch (error) {
